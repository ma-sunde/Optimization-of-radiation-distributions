...
class SaveStudyCallback:
    ...
    def __call__(self, study: optuna.study.Study, trial: optuna.trial.FrozenTrial) -> None:
        i = trial.number
        if i % 50 == 0:
            joblib.dump(study, f"C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/Study_{i}.pkl")
...
class SaveBestResultCallback:
    ...
    def __call__(self, study, trial):
        if study.best_trial == trial:
            ...
            df_result.to_csv(f"C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/Object_Detection_Best_Results_{i}.csv", index=False)
...
def objective(trial):
    ...
    # Best parameters from previous
    ld2_it2 = 0.11051667017884471
    ld3_it2 = 0.5380285686118893
    ld4_it2 = 0.5698022270077476
    ...
    ld2_it3 = 0.11309006000553078
    ld3_it3 = 0.394014203188904
    ld4_it3 = 0.6857106390347636
    
    # Create the four 4x4 arrays with the optimized values
    arr2_it3 = np.full((4, 4), ld2_it3)
    arr3_it3 = np.full((4, 4), ld3_it3)
    arr4_it3 = np.full((4, 4), ld4_it3)

    # Define the light distribution as a set of hyperparameters
    ld2_it4 = 0.21305832342484926
    ld3_it4 = 0.04292122264901409
    ld4_it4 = 0.06080140065377147
    
    # Create the four 4x4 arrays with the optimized values
    arr2_it4 = np.full((2, 2), ld2_it4)
    arr3_it4 = np.full((2, 2), ld3_it4)
    arr4_it4 = np.full((2, 2), ld4_it4)

    # Define the light distribution as a set of hyperparameters
    ld1_it5 = trial.suggest_uniform("ld1_it4", 0, 1)
    ld2_it5 = trial.suggest_uniform("ld2_it4", 0, 1)
    ld3_it5 = trial.suggest_uniform("ld3_it4", 0, 1)
    ld4_it5 = trial.suggest_uniform("ld4_it4", 0, 1)
    
    # Create the four 4x4 arrays with the optimized values
    arr1_it5 = np.full((1, 1), ld1_it5)
    arr2_it5 = np.full((1, 1), ld2_it5)
    arr3_it5 = np.full((1, 1), ld3_it5)
    arr4_it5 = np.full((1, 1), ld4_it5)

     # Create arr_it2
    arr_it5 = np.concatenate([np.concatenate([arr1_it5, arr2_it5], axis=1), np.concatenate([arr3_it5, arr4_it5], axis=1)], axis=0)

    # Create arr_it2
    arr_it4 = np.concatenate([np.concatenate([arr_it5, arr2_it4], axis=1), np.concatenate([arr3_it4, arr4_it4], axis=1)], axis=0)

    # Create arr_it2
    arr_it3 = np.concatenate([np.concatenate([arr_it4, arr2_it3], axis=1), np.concatenate([arr3_it3, arr4_it3], axis=1)], axis=0)

    # Concatenate the four 4x4 arrays into an 8x8 array
    arr_it2= np.concatenate(
        [np.concatenate([arr_it3, arr2_it2], axis=1), np.concatenate([arr3_it2, arr4_it2], axis=1)], axis=0
    )

    # Concatenate the four 4x4 arrays into an 8x8 array
    arr_it0= np.concatenate(
        [np.concatenate([arr_it2, arr2_it1], axis=1), np.concatenate([arr3_it1, arr4_it1], axis=1)], axis=0
    )

    # Concatenate the four 4x4 arrays into an 8x8 array
    light_distribution = np.concatenate(
        [np.concatenate([arr1, arr2], axis=1), np.concatenate([arr3, arr_it0], axis=1)], axis=0
    )
...
if __name__ == "__main__":
    #log_dir = r"C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/TB_Log"

    study = optuna.create_study(sampler=TPESampler(n_startup_trials=50))
    study.optimize(
        objective,
        n_trials=500,
    ...
    joblib.dump(study, "C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/Study.pkl")
    ...
    # Write images
    fig_acc.write_image("C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/Graphs/fig_acc.svg")
    fig_hist.write_image("C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/Graphs/fig_hist.svg")
    fig_importance.write_image("C:/Users/grabe/Documents/Rayen/light_distribution_optimization/optuna/studies_advanced/Study_Level_5_Startup/Graphs/fig_importance.svg")
