...
class LightOptEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #################
        # For loading and calcualting Optimization
        #################
        # Calculate f_min
        ...
        for i in range(len(self.pos_list)):
            # Sum the accuracy
            total_err_acc += W_ped * W_pos[i] * (1 - next_ped_acc[i]) + W_car * W_pos[i] * (
                1 - next_car_acc[i]
            )
        
        info = {"total_err_acc": total_err_acc, "next_ped_acc": next_ped_acc, "next_car_acc": next_car_acc}
    ...
    def calc_reward(self, action):
        ...
        for i in range(len(self.pos_list)):
            ...
            if e_p_next <= 0.2 and e_c_next <= 0.2:
                if e_p_next <= 0.1 and e_c_next <= 0.1:
                    part_reward += 10
                else:
                    part_reward += 5
        ...
        print("Reward before self_count/action/20:", reward)
        ...
        print("Reward after self_count/action/20:", reward)
